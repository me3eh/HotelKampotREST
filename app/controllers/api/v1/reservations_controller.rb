class Api::V1::ReservationsController < ApplicationController
  ...
  def log_action
    token = request.headers[:authorization]&.gsub("Basic ", "")

    if token.nil? || Base64.decode64(token) != "maciek:macius"
      return render json: { message: "Not proper token ;P" }, status: :not_acceptable
    end
    ...
    reservation_hash =
      {
        ...
        from: reservation.from,
        to: reservation.to,
        link_to: "#{request.protocol}#{request.host_with_port}#{api_v1_reservation_path(reservation.id)}"
      }
